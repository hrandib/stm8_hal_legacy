...
#define WWDG_CR_RESET_VALUE ((uint8_t))
...
/**
  * @brief  CPU Registers (CPU)
  */

typedef struct CPU_struct
{
  __IO uint8_t A;     // Accumulator
  __IO uint8_t PCE;   // Program counter extended
  __IO uint8_t PCH;   // Program counter high
  __IO uint8_t PCL;   // Program counter low
  __IO uint8_t XH;    // X high
  __IO uint8_t XL;    // X low
  __IO uint8_t YH;    // Y High
  __IO uint8_t YL;    // Y low
  __IO uint8_t SPH;   // Stack pointer high
  __IO uint8_t SPL;   // Stack pointer low
  __IO uint8_t CCR;   // Global configuration register
}
CPU_TypeDef;
...
#define CPU ((CPU_TypeDef *) CPU_BaseAddress)
